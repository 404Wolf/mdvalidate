...
/// Represents the extracted contents of a markdown codeblock.
pub struct CodeblockContents {
    pub lang: Option<(String, usize)>,
    pub code: (String, usize),
}

impl CodeblockContents {
    ...
    /// # Returns
    ///
    /// An `Option` containing `CodeblockContents` with:
    ///
    ...
    pub fn try_from_cursor(cursor: &TreeCursor, src: &str) -> Result<Option<Self>, ValidationError> {
        ...
        Ok(Some(CodeblockContents { lang: language, code: body }))
    }
}
...
pub fn validate_str(schema: &str, input: &str) -> (serde_json::Value, Vec<ValidationError>) {
    use crate::mdschema::validation::validator::Validator;
...
mod tests {
    #[allow(unused_imports)]
    use crate::mdschema::validation::ts_types::*;
    use crate::mdschema::validation::utils::parse_markdown_and_get_tree;
