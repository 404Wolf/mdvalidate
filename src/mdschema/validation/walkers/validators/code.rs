...
use crate::mdschema::validation::ts_utils::CodeblockContents;
...
fn validate_code_vs_code_impl(walker: &ValidatorWalker) -> ValidationResult {
    ...
    let input_extracted =
        match CodeblockContents::try_from_cursor(&input_cursor, walker.input_str()) {
            Ok(value) => value,
            Err(error) => {
    ...
    let schema_extracted =
        match CodeblockContents::try_from_cursor(&schema_cursor, walker.schema_str()) {
            Ok(value) => value,
            Err(error) => {
                result.add_error(error);
                return result;
    ...
    let (Some(schema_contents), Some(input_contents)) = (&schema_extracted, &input_extracted)
    else {
        #[cfg(feature = "invariant_violations")]
        ...
        invariant_violation!(
            ...
            &schema_cursor,
            &input_cursor,
            "Failed to extract code block contents from schema or input"
        );
        #[allow(unreachable_code)]
        return result;
    };

    let (schema_lang, (schema_code, schema_code_descendant_index)) =
        (&schema_contents.lang, &schema_contents.code);
    let (input_lang, (input_code, input_code_descendant_index)) =
        (&input_contents.lang, &input_contents.code);
