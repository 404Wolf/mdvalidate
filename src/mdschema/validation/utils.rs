...
#[cfg(test)]
mod tests {
    use super::{join_values, parse_markdown_and_get_tree, Value};
    ...
    fn test_extract_codeblock_contents() {
        ...
        let contents = CodeblockContents::try_from_cursor(&cursor, input)
            .unwrap()
            .expect("expected codeblock contents");
        assert_eq!(contents.lang, None);
        assert_eq!(contents.code, ("code".into(), 3));
        ...
        let contents = CodeblockContents::try_from_cursor(&cursor, input)
            .unwrap()
            .expect("expected codeblock contents");
        assert_eq!(contents.lang, Some(("rust".into(), 3)));
        assert_eq!(contents.code, ("code".into(), 5));
    ...
    fn test_extract_codeblock_contents_multiline() {
        ...
        let contents = CodeblockContents::try_from_cursor(&cursor, input)
            .unwrap()
            .expect("expected codeblock contents");
        assert_eq!(contents.lang, Some(("python".into(), 3)));
        assert_eq!(
            contents.code,
            (
                "def hello():\n    print(\"world\")\n    return True".into(),
