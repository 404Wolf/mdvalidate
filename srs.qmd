...
We will be building a DSL (domain-specific-language) for validating Markdown,
and an associated tool that you can run to process Markdown and check whether
it conforms to a schema.

Our tool will initially take the form of a CLI, and then may offer editor
integration, in a way that looks somewhat like the following:

```{mermaid}
graph LR
     A[".MDSchema file"] --> B["./mdvalidate"]
      C["Markdown file"] --> B
...
into existence, since Markdown is a paricuarly easy to parse and generate
...
- The user is able to define a schema for a Markdown document.
- The user is able to validate a Markdown document against a schema.
- The user is able to generate a Markdown document from a schema and data.


Further features include:
- VS code integration: after installation the user would be able to validate Markdown files against their schemas in VS code and errors will show underlined by a red squiggly line.

## User Classes and Characteristics {#user-classes}

**SWE in Big Tech Companies**:
These users, frequently write technical documentation, API specifications, and design documents in Markdown format. They need standardized schemas to ensure their documentation is consistently structured, machine-readable, and easily consumable by AI systems for automated processing, code generation, and knowledge extraction.

**AI**
AI systems increasingly require structured data formats for agentic behavior - autonomous decision-making and task execution workflows that are currently implemented using JSON schemas. However, Markdown offers significant advantages in token efficiency, making it a more cost-effective alternative for AI operations that process large volumes of structured content. By providing schema validation for Markdown documents, this tool enables AI systems to both enforce reproducible data structures for language model interactions and generate consistent documentation from schema-driven data, potentially reducing token costs and improving processing efficiency in high-frequency AI applications.
...
maintain interfaces that are suitable for human and machine interpretation.
...
content, and then recompute the AST as the text changes.
...
**Priority: High** - DSL for defining Markdown document structure and constraints.

### Stimulus/Response Sequences
- User creates `.MDSchema` file → Schema parsed and validated
- Invalid schema syntax → Error messages with line numbers

### Functional Requirements
- Support required/optional Markdown elements (headers, lists, code blocks)
- Define heading hierarchy constraints 
- Returns clear error messages for malformed schemas

## Feature 2: Command Line Validator Tool

### Description and Priority  
**Priority: Medium** - CLI tool that validates Markdown files against schemas.

### Stimulus/Response Sequences
- `mdvalidate schema.mdschema document.md` → Validation report
- `cat document.md | mdvalidate schema.mdschema` → Stream validation
...
- Complete validation within 20ms for 1.3k characters

## Feature 3: Language Server (LSP) Integration

### Description and Priority
**Priority: High** - LSP server for real-time Markdown validation.
...
### Functional Requirements
- Provide real-time diagnostics with error locations and messages
- Auto-detect schema files in project directories
- Handle multiple Markdown files simultaneously
...
**Speed** 
the current validation time for a json schema is about 8 ms for a 1.3k
character. Speed is especially important for AI systems that need to validate
large volumes of content in real-time. So we aim to be at least under 20 ms for
a 1.3k character Markdown file.

**Reliability**
The tool should be reliable and able to handle large volumes of content. So we
aim to be at least 99.999% reliable.

**Scalability**
The tool should be scalable and able to handle large volumes of content. So we
aim to be able to handle at least 100k characters.

**Usability**
So we aim to be able to use the tool with 2 to 3 commands.
...
- Handle corrupted or extremely large files gracefully without system crashes
  or memory exhaustion

- Ensure robust error handling that fails safely when encountering unexpected
  input formats
- Isolate validation processes to prevent cascading failures affecting other
  system components
...
Define any security or privacy certifications that must be satisfied. -->
  ...
  - We dont have security or privacy issues yet. We are not going to store or
...
connection and a simple command line interface (accessible via nix packages or
via brew)
