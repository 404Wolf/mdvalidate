...
`mdvalidate` ia a tool that allows the user to validate the shape of a Markdown
by declaring a schema and an input Markdown file.

== Testing Environment

x86_64 Linux github action test runner in a Nix development environment

We are developing in a Nix development shell defined by the following (subject
to change)
...
We will also have a overall end to end test to actually ensure the functionality
of the command line tool. This will take the form of a simple test script that
...
- `mdv schema.md input.md` (where `schema.md` is the schema file, shown above, and `input.md` the
  input file, also shown above).
...
#table(
  ...
  - Opening and closing a file rapidly (sending many `textDocument/didOpen`s for the
    ...
    - User deletes a chunk of text all of a sudden and the remaining text is invalid
      Markdown (*does not* conform to schema but also just is bad Markdown)
    - User deletes a chunk of text all of a sudden and the remaining text is valid
      Markdown (*does not* conform to schema but is bad Markdown)
    - User deletes a chunk of text all of a sudden and the remaining text is valid
      Markdown (*does* conform to schema but is bad Markdown)
    - User adds a chunk of text all of a sudden and the remaining text is invalid
      Markdown (*does not* conform to schema but also just is bad Markdown)
    ...
    - User closes a file that has invalid Markdown (*does not* conform to schema but
      also just is bad Markdown)
    - User closes a file that has valid Markdown (*does not* conform to schema but is
      bad Markdown)
    - User closes a file that has valid Markdown (*does* conform to schema but is bad
      Markdown)
    ...
    - Publishing diagnostics for a node type mismatch (e.g., expected heading, found
      paragraph)
    ...
    - Publishing diagnostics for nested structural errors (e.g., list item within
      wrong list type)
...
We also implemented structural testing to ensure that the codebase is covered by
tests.
...
#figure(image("images/flamegraph.svg")) <fig:flamegraph>
