...
This document is written in #link("typst", "https://typst.app/"), a scientific
and technical publishing system.

We assume knowledge of some surrounding technologies like abstract syntax trees,
but defines more specific terms in the glossary section at the end.
...
performs validation, providing useful information about whether it conforms to a
shape in a schema.

#link("Markdown", "https://learnxinyminutes.com/markdown/") is an unstructured
superset of html that is easy to read and write for humans. It is widely used
for technical documentation, note-taking, and more. However, because it is
...
The beauty of defining a schema in such a way that every Markdown AST node has a
clear definition and label is we will also be able to go from a structured data
format like JSON or YAML to a Markdown document that conforms to the schema. For
example, given the schema above and the JSON data
...
- Use in other tools that allow users to edit arbitrary document formats as
  ...
  to JSON, then you define a Markdown schema that can carry all the fields of the
  JSON, then you can make it possible to "edit" those types of files in Markdown
  in a way where it can easily be converted back).
...
Some notable ones include linters, like #link("markdownlint", "https://github.com/DavidAnson/markdownlint"),
that can validate Markdown. It will tell you if (and where) you are missing
closing tags, for example, or have bad spacing alignment. There are also
formatting tools for markdown like [Prettier](https://prettier.io/) that can
format markdown to a unified style.
...
#link("MarkItDown", "https://github.com/microsoft/markitdown"), a tool that can
convert many different inputs, like PDFs, word documents, and more to Markdown.
There are also traditional document conversion tools, like
...
- Editor integration via Language Server Protocol. This lets users of MDvalidate
  validate Markdown files against their schemas in editors that support LSP
  protocol (like VSCode and Neovim) identify errors by a red squiggly lines as
  they edit their Markdown.

== User Classes and Characteristics

_Technical Hobbiests:_

These users are technical users who use Markdown on a daily basis to take notes,
and generally prefer text-based document formats. Many of these users may be
taking notes in markdown (e.g., Obsidian, Logseq, etc) and want to ensure that
their notes conform to a certain structure for better organization and
retrieval. Some will be developers looking to build meta-tooling incorporating
Markdown validation into their own applications.

_Corporate software engineers/Companies_:
...
operations that process large volumes of structured content. By providing schema
validation for Markdown documents, this tool enables AI systems to both enforce
reproducible data structures for language model interactions and generate
consistent documentation from schema-driven data, potentially reducing token
costs and improving processing efficiency in high-frequency AI applications.
...
We will provide generated documentation using #link("https://clap.rs/", "clap") for
the command line interface, and will also provide a simple Markdown `README.md` on
...
#link("https://commonmark.org/", "CommonMark") specification of Markdown, which
is a well-defined and widely adopted standard. We will also strive to have
support for GitHub Flavored Markdown (GFM), additions, like Markdown tables.
...
We will use #link("Clap", "https://github.com/clap-rs/clap") to build the
command line interface and do argument parsing, which is one of the most common
rust CLI libraries.

We will also use many common Rust development tools like #link("Machete", "https://github.com/bnjbvr/cargo-machete") and `cargo` tools.

To enable incremental parsing, which is important for streamed input and
real-time validation, we will use #link("Treesitter", "https://tree-sitter.github.io/tree-sitter/").
Treesitter is an incremental parsing library that allows us to generate an AST
for given text content, and then recompute the AST as the text changes. We may
even be able to define a grammer for our DSL with Treesitter grammers as well to
make processing our schema files easier.
...
#link("https://github.com/ebkalderon/tower-lsp", "Tower LSP"), which is a Rust
library to build language servers. It is used by popular language LSPs like
[Deno's language server](https://docs.deno.com/runtime/reference/cli/lsp/).

To package our program, we will use #link("https://nixos.org/", "Nix") for
builds and deployment/CI.
...
- User creates `.mds` (markdown-schema) file -> Schema parsed and validated by
  MDvalidate
...
- Handle corrupted or extremely large files gracefully without crashing or memory
  exhaustion
...
Our program is designed to provide security guarantees. It's highly critical
...
We will distribute MDvalidate via the #link("the Nix package repository", "https://search.nixos.org/packages"),
...
- _LSP_: Language server protocol, a protocol for language servers to communicate
  with editors and IDEs.
- _AST_: Abstract syntax tree, a tree representation of the abstract syntactic
  structure of source code.
- _DSL_: Domain specific language, a programming language or specification
...
- _MDvalidate_: The name of the Markdown validation tool we are building.
- _Markdown_: A lightweight markup language with plain text formatting syntax.
- _Schema_: A formal definition of the structure and constraints of a document or
  data format that MDvalidate will use to validate Markdown documents.
- _CommonMark_: A strongly defined, highly compatible specification of Markdown.
- _GFM_: GitHub Flavored Markdown, a dialect of Markdown that adds some features
  to the original Markdown specification, like tables.
- _Regex_: Regular expression, a sequence of characters that define a search
  pattern, often used for string matching within texts.
